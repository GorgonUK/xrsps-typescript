/**
 * Pre-import gate for Phase 7B-2 Slice 2A (sub-chamber NPC kill disambiguation).
 *
 * Usage: npx tsx server/scripts/leagues/verify-phase7b2-slice2a-candidates.ts
 */
import fs from "fs";
...
import {
    PHASE7B2_SLICE2A_APPROVED,
    PHASE7B2_SLICE2A_REGION_BOUNDS,
    PHASE7B2_SLICE2A_SOURCE_IDS,
    type Phase7b2Slice2aBounds,
} from "./lib/phase7b2Slice2aApproved";
...
function main(): void {
    ...
    console.log(
        `[verify-phase7b2-slice2a-candidates] Phase 7B-2 Slice 2A gate (${PHASE7B2_SLICE2A_SOURCE_IDS.length} CSV ids)\n`,
    );

    for (const approved of PHASE7B2_SLICE2A_APPROVED) {
        ...
        const bounds = PHASE7B2_SLICE2A_REGION_BOUNDS[sourceTaskId];
        if (!bounds) {
        ...
        tierA.push({
            ...
            note: `sub-chamber: ${approved.boundsProof}`,
            confidence: approved.confidence,
            trigger: { type: "npc_kill", npcIds: [approved.chosenNpcId] },
        });
    }

    if (tierA.length !== PHASE7B2_SLICE2A_SOURCE_IDS.length) {
        ...
        console.error(
            `\n[verify-phase7b2-slice2a-candidates] FAIL: expected ${PHASE7B2_SLICE2A_SOURCE_IDS.length}, got ${tierA.length}`,
        );
        process.exit(1);
    }
    ...
    for (const sourceTaskId of PHASE7B2_SLICE2A_SOURCE_IDS) {
        const live = existingTasks.find((t) => t.sourceTaskId === sourceTaskId && t.mvpTaskId !== undefined);
        if (live) {
            console.error(
                `[verify-phase7b2-slice2a-candidates] FAIL: CSV ${sourceTaskId} already has mvpTaskId in manifest`,
            );
            process.exit(1);
        }
    }
    ...
    const otherPending = existingTasks.filter(
        (t) => t.mvpTaskId === undefined && t.slice !== "2a",
    );
    const manifest = {
        description:
    ...
    console.log(`\n[verify-phase7b2-slice2a-candidates] Wrote ${manifestPath} (+${tierA.length} slice 2a)`);
    console.log(`[verify-phase7b2-slice2a-candidates] PASS: ${tierA.length} tasks for import`);
    console.log(
        `[verify-phase7b2-slice2a-candidates] Expected live after import: ${LEAGUE_TASKS.length + tierA.length}`,
    );
}
