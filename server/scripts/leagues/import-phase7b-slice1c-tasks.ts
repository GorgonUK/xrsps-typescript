/**
 * Phase 7B-1c: append 3+ candidate boss tasks with spawned canonical npc_kill ids.
 *
 * Usage: npx tsx server/scripts/leagues/import-phase7b-slice1c-tasks.ts
 */
import fs from "fs";
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + phases through 7B-1c). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    const slice1c = manifest.tasks.filter(
        (t) => t.slice === "1c" && SLICE1C_SOURCE_IDS.includes(t.sourceTaskId as (typeof SLICE1C_SOURCE_IDS)[number]),
    );
    if (slice1c.length !== SLICE1C_SOURCE_IDS.length) {
        console.error(
            `[phase7b-slice1c] Run verify-phase7b1c-candidates.ts first (have ${slice1c.length}, need ${SLICE1C_SOURCE_IDS.length})`,
        );
        process.exit(1);
    }
    ...
    const toImport = slice1c.filter((t) => !liveSourceIds.has(t.sourceTaskId | 0));

    for (const sourceId of SLICE1C_SOURCE_IDS) {
        if (liveSourceIds.has(sourceId)) {
            console.error(`[phase7b-slice1c] sourceTaskId ${sourceId} already live`);
            process.exit(1);
        }
        const csv = csvById.get(sourceId);
        if (!csv) {
            console.error(`[phase7b-slice1c] Missing CSV row ${sourceId}`);
            process.exit(1);
        }
        if (liveNames.has(csv.name.trim())) {
            console.error(`[phase7b-slice1c] live name collision: "${csv.name.trim()}" (CSV ${sourceId})`);
            process.exit(1);
        }
        ...
        if (!trig || trig.type !== "npc_kill" || trig.npcIds.length !== 1) {
            console.error(`[phase7b-slice1c] Invalid single npc_kill trigger for ${sourceId}`);
            process.exit(1);
        }
    }

    const importNames = SLICE1C_SOURCE_IDS.map((id) => csvById.get(id)!.name.trim());
    const dupName = importNames.find((n, i) => importNames.indexOf(n) !== i);
    if (dupName) {
        console.error(`[phase7b-slice1c] duplicate task name in import batch: "${dupName}"`);
        process.exit(1);
    }

    console.log(
        `[phase7b-slice1c] Pre-import sanity: ${toImport.length} slice-1c tasks, 0 live duplicates`,
    );
    ...
    for (const sourceId of SLICE1C_SOURCE_IDS) {
        ...
        if (assignedMvpIds.has(mvpTaskId)) {
            console.error(`[phase7b-slice1c] mvpTaskId collision: ${mvpTaskId}`);
            process.exit(1);
        }
    ...
    if (importedSourceIds.length === 0) {
        console.log("[phase7b-slice1c] Nothing to import (all slice 1c tasks already have mvpTaskId)");
        process.exit(0);
    }
    ...
    console.log(
        `[phase7b-slice1c] Imported ${importedSourceIds.length} tasks (taskId ${LEAGUE_TASKS.length}..${rows.length - 1}, total ${rows.length})`,
    );
    console.log(`[phase7b-slice1c] CSV source ids: ${importedSourceIds.join(", ")}`);
}
