/**
 * Phase 7A-2 Slice 3: append mixed registry/allowlist Tier B collection tasks (8 tasks).
 *
 * Usage: npx tsx server/scripts/leagues/import-phase7a-slice3-tasks.ts
 */
import fs from "fs";
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + phases through 7A-2 Slice 3). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    const slice = manifest.tasks.filter(
        (t) => t.tier === "B-3" && SLICE3_SOURCE_IDS.includes(t.sourceTaskId as (typeof SLICE3_SOURCE_IDS)[number]),
    );
    if (slice.length !== SLICE3_SOURCE_IDS.length) {
        console.error(
            `[phase7a-slice3] Run verify-phase7a2-slice3-candidates.ts first (have ${slice.length}, need ${SLICE3_SOURCE_IDS.length})`,
        );
        process.exit(1);
    }
    ...
    for (const sourceId of SLICE3_SOURCE_IDS) {
        if (liveSourceIds.has(sourceId)) {
            console.error(`[phase7a-slice3] sourceTaskId ${sourceId} already live`);
            process.exit(1);
        }
        const csv = csvById.get(sourceId);
        if (!csv) {
            console.error(`[phase7a-slice3] Missing CSV row ${sourceId}`);
            process.exit(1);
        }
        if (liveNames.has(csv.name.trim())) {
            console.error(`[phase7a-slice3] live name collision: "${csv.name.trim()}" (CSV ${sourceId})`);
            process.exit(1);
        }
        ...
        if (!trig || trig.type !== "item_equip" || trig.itemIds.length !== 1) {
            console.error(`[phase7a-slice3] Invalid single item_equip trigger for ${sourceId}`);
            process.exit(1);
        }
        const allow = getLeagueObtainAllowlistEntry(sourceId);
        if (allow && JSON.stringify(trig) !== JSON.stringify(allow.trigger)) {
            console.error(`[phase7a-slice3] Manifest/allowlist trigger mismatch for ${sourceId}`);
            process.exit(1);
        }
    }

    console.log(`[phase7a-slice3] Pre-import sanity: ${SLICE3_SOURCE_IDS.length} tasks, 0 live duplicates`);
    ...
    for (const sourceId of SLICE3_SOURCE_IDS) {
        ...
        if (assignedTaskIds.has(mvpTaskId)) {
            console.error(`[phase7a-slice3] taskId collision: ${mvpTaskId}`);
            process.exit(1);
        }
    ...
    if (importedSourceIds.length === 0) {
        console.log("[phase7a-slice3] Nothing to import (all tasks already have mvpTaskId)");
        process.exit(0);
    }
    ...
    console.log(
        `[phase7a-slice3] Imported ${importedSourceIds.length} tasks (taskId ${LEAGUE_TASKS.length}..${rows.length - 1}, total ${rows.length})`,
    );
    console.log(`[phase7a-slice3] CSV source ids: ${importedSourceIds.join(", ")}`);
}
