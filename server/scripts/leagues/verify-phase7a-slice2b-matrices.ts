/**
 * Static item_equip matrix checks for Phase 7A-2 Slice 2.
 *
 * Usage: npx tsx server/scripts/leagues/verify-phase7a-slice2b-matrices.ts
 */
import fs from "fs";
...
function main(): void {
    ...
    console.log("[verify-phase7a-slice2b-matrices] Phase 7A-2 Slice 2 item_equip checks\n");

    const matrixSamples: Array<{ label: string; sourceTaskId: number; itemId: number; altItemId: number }> = [
        { label: "Primordial Boots", sourceTaskId: 1503, itemId: 13239, altItemId: 13240 },
        { label: "Pegasian Boots", sourceTaskId: 1504, itemId: 13237, altItemId: 13238 },
        { label: "Eternal Boots", sourceTaskId: 1505, itemId: 13235, altItemId: 13236 },
        { label: "Necklace of Anguish", sourceTaskId: 1579, itemId: 19547, altItemId: 19548 },
        { label: "Tormented Bracelet", sourceTaskId: 1580, itemId: 19544, altItemId: 19545 },
        { label: "Ring of Suffering", sourceTaskId: 1581, itemId: 19550, altItemId: 19551 },
        { label: "Crystal Helm", sourceTaskId: 1589, itemId: 23971, altItemId: 23972 },
        { label: "Crystal Body", sourceTaskId: 1591, itemId: 23975, altItemId: 23976 },
        { label: "Kodai Wand", sourceTaskId: 1596, itemId: 21006, altItemId: 21007 },
    ];

    const extraNegativeOnly = [{ label: "Crystal Legs alt", sourceTaskId: 1590, altItemId: 23980 }];
    ...
    for (const sample of extraNegativeOnly) {
        const mvpTaskId = sourceIdToMvpTaskId(sample.sourceTaskId);
        ...
        manager.onItemEquip(1, sample.altItemId);
        if (LeagueTaskService.isTaskComplete(player, mvpTaskId)) {
            logFail(`alternate item ${sample.altItemId} incorrectly completed CSV ${sample.sourceTaskId}`);
        } else {
            logOk(`alternate item ${sample.altItemId} does not complete (${sample.label})`);
        }
    }
    ...
    if (failed > 0) {
        console.error(`\n[verify-phase7a-slice2b-matrices] ${failed} check(s) failed`);
        process.exit(1);
    }
    console.log("\n[verify-phase7a-slice2b-matrices] All Slice 2 matrix checks passed");
}
