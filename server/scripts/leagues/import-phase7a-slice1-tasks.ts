/**
 * Phase 7A-1: append Strict Tier A collection ambiguous tasks (item_equip / item_obtain).
 *
 * Usage: npx tsx server/scripts/leagues/import-phase7a-slice1-tasks.ts
 */
import fs from "fs";
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + phases through 3D/2D + Phase 7A-1). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    if (manifest.tier !== "A" || manifest.tasks.length === 0) {
        console.error("[phase7a-slice1] Run verify-phase7a1-candidates.ts first");
        process.exit(1);
    }

    const liveSourceIds = collectLiveSourceIds(csvByName);
    const importIds = manifest.tasks.map((t) => t.sourceTaskId | 0);

    for (const sourceId of importIds) {
        if (liveSourceIds.has(sourceId)) {
            console.error(`[phase7a-slice1] sourceTaskId ${sourceId} already live`);
            process.exit(1);
        }
        if (!csvById.has(sourceId)) {
            console.error(`[phase7a-slice1] Missing CSV row ${sourceId}`);
            process.exit(1);
        }
        const trig = manifest.tasks.find((t) => t.sourceTaskId === sourceId)?.trigger;
        if (!trig || (trig.type !== "item_equip" && trig.type !== "item_obtain")) {
            console.error(`[phase7a-slice1] Invalid trigger for ${sourceId}`);
            process.exit(1);
        }
    }

    console.log(
        `[phase7a-slice1] Pre-import sanity: ${importIds.length} Tier-A tasks, 0 live duplicates`,
    );
    ...
    console.log(
        `[phase7a-slice1] Imported ${importedSourceIds.length} tasks (taskId ${LEAGUE_TASKS.length}..${rows.length - 1}, total ${rows.length})`,
    );
    console.log(`[phase7a-slice1] CSV source ids: ${importedSourceIds.join(", ")}`);
}
