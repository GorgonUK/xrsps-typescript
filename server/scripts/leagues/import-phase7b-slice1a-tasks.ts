/**
 * Phase 7B-1a: append generic NPC kill Tier A tasks (npc_kill).
 *
 * Usage: npx tsx server/scripts/leagues/import-phase7b-slice1a-tasks.ts
 */
import fs from "fs";
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + phases through 7B-1a). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    const importableIds = SLICE1A_SOURCE_IDS.filter((id) => !SLICE1A_DEFERRED.has(id));
    const slice1a = manifest.tasks.filter(
        (t) => t.slice === "1a" && importableIds.includes(t.sourceTaskId | 0),
    );
    if (slice1a.length !== importableIds.length) {
        console.error(
            `[phase7b-slice1a] Run verify-phase7b1a-candidates.ts first (have ${slice1a.length}, need ${importableIds.length})`,
        );
        process.exit(1);
    }

    const liveSourceIds = collectLiveSourceIds(csvByName);
    const toImport = slice1a.filter((t) => !liveSourceIds.has(t.sourceTaskId | 0));

    for (const sourceId of importableIds) {
        if (liveSourceIds.has(sourceId)) {
            console.error(`[phase7b-slice1a] sourceTaskId ${sourceId} already live`);
            process.exit(1);
        }
        if (!csvById.has(sourceId)) {
            console.error(`[phase7b-slice1a] Missing CSV row ${sourceId}`);
            process.exit(1);
        }
        ...
        if (!trig || trig.type !== "npc_kill" || trig.npcIds.length !== 1) {
            console.error(`[phase7b-slice1a] Invalid single npc_kill trigger for ${sourceId}`);
            process.exit(1);
        }
    }

    console.log(
        `[phase7b-slice1a] Pre-import sanity: ${toImport.length} slice-1a tasks, 0 live duplicates`,
    );
    ...
    if (importedSourceIds.length === 0) {
        console.log("[phase7b-slice1a] Nothing to import (all slice 1a tasks already have mvpTaskId)");
        process.exit(0);
    }
    ...
    console.log(
        `[phase7b-slice1a] Imported ${importedSourceIds.length} tasks (taskId ${LEAGUE_TASKS.length}..${rows.length - 1}, total ${rows.length})`,
    );
    console.log(`[phase7b-slice1a] CSV source ids: ${importedSourceIds.join(", ")}`);
}
