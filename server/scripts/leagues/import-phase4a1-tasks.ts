/**
 * Phase 4A1: append spell_cast (core spell id) tasks to live league task list.
 */
import fs from "fs";
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + Phase 2/2B + Phase 3 + Phase 3C + Phase 4A1). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    const phase4a1Path = path.join(repoRoot, "server/data/leagues/phase4a1-spellcast-tasks.json");
    const phase4a1 = JSON.parse(fs.readFileSync(phase4a1Path, "utf8")) as Phase4a1File;
    ...
    for (const entry of phase4a1.tasks) {
    ...
    console.log(
        `Phase 4A1 import complete: ${tasks.length} live tasks (+${phase4a1.tasks.length} spell_cast).`,
    );
}
