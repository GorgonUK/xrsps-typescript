/**
 * Phase 3C: append combat_level_reach tasks (133+) to live league task list.
 */
import fs from "fs";
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + Phase 2/2B + Phase 3 + Phase 3C). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    const phase3cPath = path.join(repoRoot, "server/data/leagues/phase3c-combat-tasks.json");
    const phase3c = JSON.parse(fs.readFileSync(phase3cPath, "utf8")) as Phase3cFile;
    ...
    for (const entry of phase3c.tasks) {
        ...
        if (entry.trigger.type !== "combat_level_reach") {
            throw new Error(`Expected combat_level_reach for sourceTaskId ${sourceTaskId}`);
        }
    ...
    console.log(
        `Phase 3C import complete: ${tasks.length} live tasks (+${phase3c.tasks.length} combat level).`,
    );
}
