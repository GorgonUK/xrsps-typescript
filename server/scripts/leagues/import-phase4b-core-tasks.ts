/**
 * Phase 4B-Core: append spell_cast (ancient + wilderness ice) tasks to live league task list.
 */
import fs from "fs";
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + Phase 2/2B + Phase 3 + Phase 3C + Phase 4A1–4A3 + Phase 4B-Core). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    const phase4bPath = path.join(repoRoot, "server/data/leagues/phase4b-core-spellcast-tasks.json");
    const phase4b = JSON.parse(fs.readFileSync(phase4bPath, "utf8")) as Phase4bCoreFile;

    if (phase4b.tasks.length !== 3) {
        throw new Error(`Expected exactly 3 Phase 4B-Core tasks, got ${phase4b.tasks.length}`);
    }
    ...
    for (const entry of phase4b.tasks) {
        ...
        if (entry.trigger.type !== "spell_cast") {
            throw new Error(`Expected spell_cast for sourceTaskId ${sourceTaskId}`);
        }
    ...
    console.log(
        `Phase 4B-Core import complete: ${tasks.length} live tasks (+${phase4b.tasks.length} spell_cast).`,
    );
}
