/**
 * Phase 7A-2 Slice 2: append allowlist-backed Tier B collection tasks (10 tasks).
 *
 * Usage: npx tsx server/scripts/leagues/import-phase7a-slice2b-tasks.ts
 */
import fs from "fs";
...
const SLICE2B_SOURCE_IDS = [1503, 1504, 1505, 1579, 1580, 1581, 1589, 1590, 1591, 1596] as const;
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + phases through 7A-2 Slice 2). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    const slice = manifest.tasks.filter(
        (t) => t.tier === "B-2" && SLICE2B_SOURCE_IDS.includes(t.sourceTaskId as (typeof SLICE2B_SOURCE_IDS)[number]),
    );
    if (slice.length !== SLICE2B_SOURCE_IDS.length) {
        console.error(
            `[phase7a-slice2b] Run verify-phase7a2-slice2-candidates.ts first (have ${slice.length}, need ${SLICE2B_SOURCE_IDS.length})`,
        );
        process.exit(1);
    }
    ...
    for (const sourceId of SLICE2B_SOURCE_IDS) {
        if (liveSourceIds.has(sourceId)) {
            console.error(`[phase7a-slice2b] sourceTaskId ${sourceId} already live`);
            process.exit(1);
        }
        const allow = getLeagueObtainAllowlistEntry(sourceId);
        if (!allow) {
            console.error(`[phase7a-slice2b] Missing allowlist entry for ${sourceId}`);
            process.exit(1);
        }
        const csv = csvById.get(sourceId);
        if (!csv) {
            console.error(`[phase7a-slice2b] Missing CSV row ${sourceId}`);
            process.exit(1);
        }
        if (liveNames.has(csv.name.trim())) {
            console.error(`[phase7a-slice2b] live name collision: "${csv.name.trim()}" (CSV ${sourceId})`);
            process.exit(1);
        }
        ...
        if (!trig || trig.type !== "item_equip" || trig.itemIds.length !== 1) {
            console.error(`[phase7a-slice2b] Invalid single item_equip trigger for ${sourceId}`);
            process.exit(1);
        }
        if (JSON.stringify(trig) !== JSON.stringify(allow.trigger)) {
            console.error(`[phase7a-slice2b] Manifest/allowlist trigger mismatch for ${sourceId}`);
            process.exit(1);
        }
    }

    console.log(
        `[phase7a-slice2b] Pre-import sanity: ${SLICE2B_SOURCE_IDS.length} allowlist-backed tasks, 0 live duplicates`,
    );
    ...
    for (const sourceId of SLICE2B_SOURCE_IDS) {
        ...
        if (assignedTaskIds.has(mvpTaskId)) {
            console.error(`[phase7a-slice2b] taskId collision: ${mvpTaskId}`);
            process.exit(1);
        }
    ...
    if (importedSourceIds.length === 0) {
        console.log("[phase7a-slice2b] Nothing to import (all tasks already have mvpTaskId)");
        process.exit(0);
    }
    ...
    console.log(
        `[phase7a-slice2b] Imported ${importedSourceIds.length} tasks (taskId ${LEAGUE_TASKS.length}..${rows.length - 1}, total ${rows.length})`,
    );
    console.log(`[phase7a-slice2b] CSV source ids: ${importedSourceIds.join(", ")}`);
}
