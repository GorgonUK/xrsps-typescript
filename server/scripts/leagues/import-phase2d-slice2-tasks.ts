/**
 * Phase 2D-2 Slice 1: append 15 manifest-only skilling_action tasks.
 *
 * Usage: npx tsx server/scripts/leagues/import-phase2d-slice2-tasks.ts
 */
import fs from "fs";
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + Phase 2/2B + Phase 3 + Phase 3C + Phase 4A1–4A3 + Phase 4B-Core + Phase 6B + Phase 2C-A + Phase 2C-B1 + Phase 2C-A Batch 3 + Phase 3D Slice 1 + Phase 2D Slice 1 + Phase 7A/7B + Phase 2D-2 Slice 1). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    for (const sourceId of SLICE1_SOURCE_IDS) {
        if (liveSourceIds.has(sourceId)) {
            console.error(`[phase2d-slice2] sourceTaskId ${sourceId} already live`);
            process.exit(1);
        }
        if (!csvById.has(sourceId)) {
            console.error(`[phase2d-slice2] Missing CSV row ${sourceId}`);
            process.exit(1);
        }
        if (!manifestBySource.has(sourceId)) {
            console.error(`[phase2d-slice2] Missing manifest entry for ${sourceId}`);
            process.exit(1);
        }
    }

    console.log(
        `[phase2d-slice2] Pre-import sanity: ${SLICE1_SOURCE_IDS.length} unique CSV ids, 0 live duplicates`,
    );
    ...
    console.log(
        `[phase2d-slice2] Imported ${importedSourceIds.length} tasks (taskId ${LEAGUE_TASKS.length}..${rows.length - 1}, total ${rows.length})`,
    );
    console.log(`[phase2d-slice2] CSV source ids: ${importedSourceIds.join(", ")}`);
}
