/**
 * Phase 4A2: append spell_cast (teleport/count) tasks to live league task list.
 */
import fs from "fs";
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + Phase 2/2B + Phase 3 + Phase 3C + Phase 4A1 + Phase 4A2). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    const phase4a2Path = path.join(repoRoot, "server/data/leagues/phase4a2-spellcast-tasks.json");
    const phase4a2 = JSON.parse(fs.readFileSync(phase4a2Path, "utf8")) as Phase4a2File;

    if (phase4a2.tasks.length !== 13) {
        throw new Error(`Expected exactly 13 Phase 4A2 tasks, got ${phase4a2.tasks.length}`);
    }
    ...
    for (const entry of phase4a2.tasks) {
    ...
    console.log(
        `Phase 4A2 import complete: ${tasks.length} live tasks (+${phase4a2.tasks.length} spell_cast).`,
    );
}
