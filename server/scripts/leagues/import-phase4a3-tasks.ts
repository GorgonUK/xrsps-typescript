/**
 * Phase 4A3: append spell_cast (alchemy) tasks to live league task list.
 */
import fs from "fs";
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + Phase 2/2B + Phase 3 + Phase 3C + Phase 4A1–4A3). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    const phase4a3Path = path.join(repoRoot, "server/data/leagues/phase4a3-alchemy-tasks.json");
    const phase4a3 = JSON.parse(fs.readFileSync(phase4a3Path, "utf8")) as Phase4a3File;

    if (phase4a3.tasks.length !== 4) {
        throw new Error(`Expected exactly 4 Phase 4A3 tasks, got ${phase4a3.tasks.length}`);
    }
    ...
    for (const entry of phase4a3.tasks) {
    ...
    console.log(
        `Phase 4A3 import complete: ${tasks.length} live tasks (+${phase4a3.tasks.length} alchemy spell_cast).`,
    );
}
