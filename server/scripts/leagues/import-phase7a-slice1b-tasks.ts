/**
 * Phase 7A-1b: append allowlist-backed collection tasks (19 tasks).
 *
 * Usage: npx tsx server/scripts/leagues/import-phase7a-slice1b-tasks.ts
 */
import fs from "fs";
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + phases through 7A-1b). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    for (const sourceId of SLICE1B_SOURCE_IDS) {
        if (liveSourceIds.has(sourceId)) {
            console.error(`[phase7a-slice1b] sourceTaskId ${sourceId} already live`);
            process.exit(1);
        }
        const allow = getLeagueObtainAllowlistEntry(sourceId);
        if (!allow) {
            console.error(`[phase7a-slice1b] Missing allowlist entry for ${sourceId}`);
            process.exit(1);
        }
        const manifestEntry = manifest.tasks.find((t) => t.sourceTaskId === sourceId);
        if (!manifestEntry || manifestEntry.tier !== "A-1b") {
            console.error(`[phase7a-slice1b] Run verify-phase7a1b-candidates.ts first (${sourceId})`);
            process.exit(1);
        }
    }

    console.log(
        `[phase7a-slice1b] Pre-import sanity: ${SLICE1B_SOURCE_IDS.length} allowlist tasks, 0 live duplicates`,
    );
    ...
    console.log(
        `[phase7a-slice1b] Imported ${importedSourceIds.length} tasks (taskId ${LEAGUE_TASKS.length}..${rows.length - 1}, total ${rows.length})`,
    );
    console.log(`[phase7a-slice1b] CSV source ids: ${importedSourceIds.join(", ")}`);
}
