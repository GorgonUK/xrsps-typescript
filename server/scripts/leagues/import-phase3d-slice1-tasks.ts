/**
 * Phase 3D Slice 1: append 2 proven area_enter tasks (Crafting Guild, Warriors' Guild).
 *
 * Usage: npx tsx server/scripts/leagues/import-phase3d-slice1-tasks.ts
 */
import fs from "fs";
...
const SLICE1_SOURCE_IDS = [779, 781] as const;
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + Phase 2/2B + Phase 3 + Phase 3C + Phase 4A1–4A3 + Phase 4B-Core + Phase 6B + Phase 2C-A + Phase 2C-B1 + Phase 2C-A Batch 3 + Phase 3D Slice 1). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    for (const sourceId of SLICE1_SOURCE_IDS) {
        if (liveSourceIds.has(sourceId)) {
            console.error(`[phase3d-slice1] sourceTaskId ${sourceId} already live`);
            process.exit(1);
        }
        if (!csvById.has(sourceId)) {
            console.error(`[phase3d-slice1] Missing CSV row ${sourceId}`);
            process.exit(1);
        }
    }

    console.log(
        `[phase3d-slice1] Pre-import sanity: ${SLICE1_SOURCE_IDS.length} unique CSV ids, 0 live duplicates`,
    );
    ...
    for (const sourceTaskId of SLICE1_SOURCE_IDS) {
        const entry = manifest.tasks.find((t) => t.sourceTaskId === sourceTaskId);
        if (!entry) {
            console.error(`[phase3d-slice1] Missing manifest entry for ${sourceTaskId}`);
            process.exit(1);
        }
    ...
    console.log(
        `[phase3d-slice1] Imported ${importedSourceIds.length} tasks (taskId ${LEAGUE_TASKS.length}..${rows.length - 1}, total ${rows.length})`,
    );
    console.log(`[phase3d-slice1] CSV source ids: ${importedSourceIds.join(", ")}`);
}
