/**
 * Phase 2C-B1: append 18 verified thieving/pickpocket tasks to live league task list.
 */
import fs from "fs";
...
import { PICKPOCKET_NPCS } from "../../src/game/scripts/modules/thieving";
...
import { resetPhase2b1PickpocketTriggerCache } from "./lib/phase2b1PickpocketTriggers";
...
/** CSV sourceTaskId → index(es) into PICKPOCKET_NPCS. */
const SOURCE_TO_DEF_INDICES: Record<number, number | number[]> = {
    41: 0,
    42: 1,
    43: [2, 3],
    44: 4,
...
const IMPORT_SOURCE_IDS = Object.keys(SOURCE_TO_DEF_INDICES)
    .map(Number)
    .sort((a, b) => a - b);

function npcIdsForDefIndices(indices: number | number[]): number[] {
    const idxList = Array.isArray(indices) ? indices : [indices];
    const ids = new Set<number>();
    for (const idx of idxList) {
        const def = PICKPOCKET_NPCS[idx];
        if (!def) {
            throw new Error(`PICKPOCKET_NPCS missing index ${idx}`);
        }
...
function buildPickpocketTrigger(sourceTaskId: number): TaskTrigger {
    const indices = SOURCE_TO_DEF_INDICES[sourceTaskId];
    if (indices === undefined) {
        throw new Error(`No PICKPOCKET_NPCS mapping for sourceTaskId ${sourceTaskId}`);
    }
    return {
        type: "skilling_action",
        skill: "thieving",
        action: "pickpocket",
        targetIds: npcIdsForDefIndices(indices),
    };
}
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + Phase 2/2B + Phase 3 + Phase 3C + Phase 4A1–4A3 + Phase 4B-Core + Phase 6B + Phase 2C-A + Phase 2C-B1). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    const manifestPath = path.join(repoRoot, "server/data/leagues/phase2b1-pickpocket-tasks.json");

    const manifestTasks = IMPORT_SOURCE_IDS.map((sourceTaskId) => ({
        sourceTaskId,
        trigger: buildPickpocketTrigger(sourceTaskId),
    }));

    const manifest = {
        description: "Phase 2C-B1: verified thieving/pickpocket skilling_action triggers (18 tasks).",
        tasks: manifestTasks,
    };
    ...
    for (const entry of manifestTasks) {
        const csv = csvById.get(entry.sourceTaskId);
        if (!csv) {
            console.error(`[phase2b1] Missing CSV row ${entry.sourceTaskId}`);
            process.exit(1);
        }
    ...
    resetPhase2b1PickpocketTriggerCache();

    console.log(
        `[phase2b1] Imported ${manifestTasks.length} tasks (taskId ${LEAGUE_TASKS.length}..${rows.length - 1}, total ${rows.length})`,
    );
    console.log(`[phase2b1] CSV source ids: ${importedSourceIds.join(", ")}`);
}
