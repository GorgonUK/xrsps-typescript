/**
 * Phase 7B-2 Slice 2A: append sub-chamber NPC kill disambiguation tasks (npc_kill).
 *
 * Usage: npx tsx server/scripts/leagues/import-phase7b2-slice2a-tasks.ts
 */
import fs from "fs";
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + phases through 7B-2 Slice 2A). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    const slice2a = manifest.tasks.filter(
        (t) =>
            t.slice === "2a" &&
            PHASE7B2_SLICE2A_SOURCE_IDS.includes(t.sourceTaskId as (typeof PHASE7B2_SLICE2A_SOURCE_IDS)[number]),
    );
    if (slice2a.length !== PHASE7B2_SLICE2A_SOURCE_IDS.length) {
        console.error(
            `[phase7b2-slice2a] Run verify-phase7b2-slice2a-candidates.ts first (have ${slice2a.length}, need ${PHASE7B2_SLICE2A_SOURCE_IDS.length})`,
        );
        process.exit(1);
    }
    ...
    for (const sourceId of PHASE7B2_SLICE2A_SOURCE_IDS) {
        if (liveSourceIds.has(sourceId)) {
            console.error(`[phase7b2-slice2a] sourceTaskId ${sourceId} already live`);
            process.exit(1);
        }
        const csv = csvById.get(sourceId);
        if (!csv) {
            console.error(`[phase7b2-slice2a] Missing CSV row ${sourceId}`);
            process.exit(1);
        }
        if (liveNames.has(csv.name.trim())) {
            console.error(`[phase7b2-slice2a] live name collision: "${csv.name.trim()}" (CSV ${sourceId})`);
            process.exit(1);
        }
        ...
        if (!trig || trig.type !== "npc_kill" || trig.npcIds.length !== 1) {
            console.error(`[phase7b2-slice2a] Invalid single npc_kill trigger for ${sourceId}`);
            process.exit(1);
        }
    }
    ...
    for (const sourceId of PHASE7B2_SLICE2A_SOURCE_IDS) {
        ...
        if (assignedMvpIds.has(mvpTaskId)) {
            console.error(`[phase7b2-slice2a] mvpTaskId collision: ${mvpTaskId}`);
            process.exit(1);
        }
    ...
    if (importedSourceIds.length === 0) {
        console.log("[phase7b2-slice2a] Nothing to import");
        process.exit(0);
    }
    ...
    console.log(
        `[phase7b2-slice2a] Imported ${importedSourceIds.length} tasks (taskId ${LEAGUE_TASKS.length}..${rows.length - 1}, total ${rows.length})`,
    );
    console.log(`[phase7b2-slice2a] CSV source ids: ${importedSourceIds.join(", ")}`);
}
