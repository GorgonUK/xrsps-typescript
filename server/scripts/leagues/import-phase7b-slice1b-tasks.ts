/**
 * Phase 7B-1b: append 2-id boss + easy regional NPC kill tasks (npc_kill).
 *
 * Usage: npx tsx server/scripts/leagues/import-phase7b-slice1b-tasks.ts
 */
import fs from "fs";
...
function formatLeagueTasksTs(rows: LeagueTaskRow[]): string {
    ...
    return (
        ...
        `// Live league tasks (${rows.length}: MVP + phases through 7B-1b). Source: tasks.csv + validation.\n` +
...
function main(): void {
    ...
    const slice1b = manifest.tasks.filter(
        (t) => t.slice === "1b" && SLICE1B_SOURCE_IDS.includes(t.sourceTaskId as (typeof SLICE1B_SOURCE_IDS)[number]),
    );
    if (slice1b.length !== SLICE1B_SOURCE_IDS.length) {
        console.error(
            `[phase7b-slice1b] Run verify-phase7b1b-candidates.ts first (have ${slice1b.length}, need ${SLICE1B_SOURCE_IDS.length})`,
        );
        process.exit(1);
    }
    ...
    const toImport = slice1b.filter((t) => !liveSourceIds.has(t.sourceTaskId | 0));

    for (const sourceId of SLICE1B_SOURCE_IDS) {
        if (liveSourceIds.has(sourceId)) {
            console.error(`[phase7b-slice1b] sourceTaskId ${sourceId} already live`);
            process.exit(1);
        }
        const csv = csvById.get(sourceId);
        if (!csv) {
            console.error(`[phase7b-slice1b] Missing CSV row ${sourceId}`);
            process.exit(1);
        }
        if (liveNames.has(csv.name.trim())) {
            console.error(`[phase7b-slice1b] live name collision: "${csv.name.trim()}" (CSV ${sourceId})`);
            process.exit(1);
        }
        ...
        if (!trig || trig.type !== "npc_kill" || trig.npcIds.length !== 1) {
            console.error(`[phase7b-slice1b] Invalid single npc_kill trigger for ${sourceId}`);
            process.exit(1);
        }
    }

    const importNames = SLICE1B_SOURCE_IDS.map((id) => csvById.get(id)!.name.trim());
    const dupName = importNames.find((n, i) => importNames.indexOf(n) !== i);
    if (dupName) {
        console.error(`[phase7b-slice1b] duplicate task name in import batch: "${dupName}"`);
        process.exit(1);
    }

    console.log(
        `[phase7b-slice1b] Pre-import sanity: ${toImport.length} slice-1b tasks, 0 live duplicates`,
    );
    ...
    for (const sourceId of SLICE1B_SOURCE_IDS) {
        ...
        if (assignedMvpIds.has(mvpTaskId)) {
            console.error(`[phase7b-slice1b] mvpTaskId collision: ${mvpTaskId}`);
            process.exit(1);
        }
    ...
    if (importedSourceIds.length === 0) {
        console.log("[phase7b-slice1b] Nothing to import (all slice 1b tasks already have mvpTaskId)");
        process.exit(0);
    }
    ...
    console.log(
        `[phase7b-slice1b] Imported ${importedSourceIds.length} tasks (taskId ${LEAGUE_TASKS.length}..${rows.length - 1}, total ${rows.length})`,
    );
    console.log(`[phase7b-slice1b] CSV source ids: ${importedSourceIds.join(", ")}`);
}
