/**
 * Pre-import gate for Phase 7B-2 Slice 1 (regional NPC kill disambiguation).
 *
 * Usage: npx tsx server/scripts/leagues/verify-phase7b2-slice1-candidates.ts
 */
import fs from "fs";
...
import {
    PHASE7B2_SLICE1_APPROVED,
    PHASE7B2_SLICE1_REGION_BOUNDS,
    PHASE7B2_SLICE1_SOURCE_IDS,
} from "./lib/phase7b2Slice1Approved";
...
function main(): void {
    ...
    console.log(
        `[verify-phase7b2-slice1-candidates] Phase 7B-2 Slice 1 gate (${PHASE7B2_SLICE1_SOURCE_IDS.length} CSV ids)\n`,
    );

    for (const approved of PHASE7B2_SLICE1_APPROVED) {
        ...
        const bounds = PHASE7B2_SLICE1_REGION_BOUNDS[sourceTaskId];
        if (!bounds) {
        ...
        for (const id of approved.candidateNpcIds) {
            const count = loadSpawnedInRegion(repoRoot, id, bounds);
        ...
        if (spawnedInRegion.length !== 1 || spawnedInRegion[0].id !== approved.chosenNpcId) {
            rejected.push({
                sourceTaskId,
                name: csv.name,
                reason: `regional spawn proof failed: ${JSON.stringify(spawnedInRegion)}`,
            });
            continue;
        }
        ...
        tierA.push({
            ...
            note: `regional: ${approved.regionProof}`,
            confidence: approved.confidence,
            trigger: { type: "npc_kill", npcIds: [approved.chosenNpcId] },
        });
    }

    if (tierA.length !== PHASE7B2_SLICE1_SOURCE_IDS.length) {
        ...
        console.error(
            `\n[verify-phase7b2-slice1-candidates] FAIL: expected ${PHASE7B2_SLICE1_SOURCE_IDS.length}, got ${tierA.length}`,
        );
        process.exit(1);
    }
    ...
    for (const sourceTaskId of PHASE7B2_SLICE1_SOURCE_IDS) {
        const live = existingTasks.find((t) => t.sourceTaskId === sourceTaskId && t.mvpTaskId !== undefined);
        if (live) {
            console.error(
                `[verify-phase7b2-slice1-candidates] FAIL: CSV ${sourceTaskId} already has mvpTaskId in manifest`,
            );
            process.exit(1);
        }
    }
    ...
    const otherPending = existingTasks.filter((t) => t.mvpTaskId === undefined && t.slice !== "2");
    const manifest = {
        description:
    ...
    console.log(`\n[verify-phase7b2-slice1-candidates] Wrote ${manifestPath} (+${tierA.length} slice 2)`);
    console.log(`[verify-phase7b2-slice1-candidates] PASS: ${tierA.length} tasks for import`);
    console.log(
        `[verify-phase7b2-slice1-candidates] Expected live after import: ${LEAGUE_TASKS.length + tierA.length}`,
    );
}
