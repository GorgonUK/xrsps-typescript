...
export function registerSmithingBarModalHandler(
    ...
    handlers.set(
        SMITHING_BAR_MODAL_GROUP_ID,
        (player: PlayerState, componentId: number, option?: string) => {
...
export function openSmithingBarModal(player: PlayerState, services: ScriptServices): void {
    ...
    services.dialog.queueWidgetEvent(pid, {
        ...
        args: [
            packUid(SMITHING_BAR_MODAL_GROUP_ID, SMITHING_BAR_MODAL_COMPONENT_FRAME),
            "Select Bar",
        ],
    });
    services.dialog.queueWidgetEvent(pid, {
        ...
        args: [
            packUid(SMITHING_BAR_MODAL_GROUP_ID, SMITHING_BAR_MODAL_COMPONENT_CLOSE),
            FONT_BOLD_12,
